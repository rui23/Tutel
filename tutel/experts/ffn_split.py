...
class FusedExpertsNetwork(torch.nn.Module):
    def __init__(self, model_dim, hidden_size_per_expert, local_experts, sharded_count, activation_fn=None, activation_fn_with_self=None, output_dim=None, has_fc1_bias=False, has_fc2_bias=False):
        super().__init__()
        ...
        self.batched_fc1_w = torch.nn.Parameter(torch.empty(local_experts, model_dim, self.hidden_size))
    ...
    def reset_parameters(self):
        with torch.no_grad():
            for i in range(self.batched_fc1_w.size(0)):
                ...
                self.batched_fc1_w[i] = fc1.weight.permute(1, 0)
    ...
    def forward(self, x, cur_split_list, ctx):
        if self.skip_expert:
            # print(1111)
            return x
        ...
        if cur_split_list[0] == 0:
            y = torch.matmul(x, batched_fc1_w)
        else:
            split_point = int(x.size(1) * cur_split_list[0])
            x1 = x[:, :split_point, :]
            x2 = x[:, split_point:, :]
            y1 = torch.matmul(x1, batched_fc1_w)
            y2 = torch.matmul(x2, batched_fc1_w)
        ...
        if cur_split_list[1] == 0:
            y = torch.matmul(y, batched_fc2_w)
        else:
            split_point = int(y.size(1) * cur_split_list[1])
            y1 = y[:, :split_point, :]
            y2 = y[:, split_point:, :]
            y1 = torch.matmul(y1, batched_fc2_w)
            y2 = torch.matmul(y2, batched_fc2_w)
            y = torch.cat([y1, y2], dim=1)
        if self.batched_fc2_bias is not None:
            y = torch.add(y, batched_fc2_bias)
        return y
