...
class FusedExpertsNetwork(torch.nn.Module):
    ...
    def forward(self, x, ctx):  # x.shape = torch.Size([num_local_experts, batch_size * num_tokens * top / (num_local_experts * a2a_ffn_overlap_degree), model_dim])
        ...
        # x.shape=torch.Size([num_local_experts, batch_size * num_tokens * top / (num_local_experts * a2a_ffn_overlap_degree), model_dim]), batched_fc1_w.shape=torch.Size([num_local_experts, hidden_size, model_dim])
        y = torch.matmul(x, batched_fc1_w.permute(0, 2, 1))
        # y.shape=torch.Size([num_local_experts, batch_size * num_tokens * top / (num_local_experts * a2a_ffn_overlap_degree), hidden_size])
        if self.batched_fc1_bias is not None:
            y = torch.add(y, batched_fc1_bias)
        y = self.activation_fn(y)
